...
HotStandby::HotStandby(
    ...
    discovery_task_.Start(
        kDiscoveryTaskName,
        {kDiscoveryInterval, {USERVER_NAMESPACE::utils::PeriodicTask::Flags::kStrong}},
        [this] { RunDiscovery(); }
    );
}
...
rcu::ReadablePtr<TopologyBase::DsnIndicesByType> HotStandby::GetDsnIndicesByType() const {
    return dsn_indices_by_type_.Read();
}

rcu::ReadablePtr<TopologyBase::DsnIndices> HotStandby::GetAliveDsnIndices() const { return alive_dsn_indices_.Read(); }
...
void HotStandby::RunDiscovery() {
    ...
    for (DsnIndex i = 0; i < host_states_.size(); ++i) {
        ...
        if (state.role == ClusterHostType::kMaster) {
            ...
            max_slave_xact_timestamp = std::max(max_slave_xact_timestamp, state.current_xact_timestamp);
        }
    }
    ...
    for (DsnIndex i = 0; i < host_states_.size(); ++i) {
        ...
        const auto target_xact_timestamp =
            master ? (slave.wal_lsn < master->wal_lsn ? master->current_xact_timestamp : slave.current_xact_timestamp)
                   : max_slave_xact_timestamp;
        // Target should be ahead of current, but clamp to zero for extra safety.
        auto slave_lag = std::max(
            ReplicationLag::zero(),
        ...
        if (topology_settings.max_replication_lag > std::chrono::milliseconds{0} &&
            ...
            for (const auto& ss_app_name : master->detected_sync_slaves) {
                if (USERVER_NAMESPACE::utils::StrIcaseEqual{}(slave.app_name, ss_app_name)) {
...
void HotStandby::RunCheck(DsnIndex idx) {
    ...
    USERVER_NAMESPACE::utils::FastScopeGuard role_check_guard([&state]() noexcept { state.Reset(); });

    if (!state.connection) {
        try {
            state.connection = MakeTopologyConnection(idx);
        } catch (const ConnectionError& e) {
            LOG_WARNING() << "Failed to connect to " << DsnCutPassword(dsn) << ": " << e;
            return;
        }
    }
    ...
    try {
        ...
        if (!state.connection->IsInRecovery()) {
            ...
            state.detected_sync_slaves = ParseSyncStandbyNames(res.AsSingleRow<std::string>());
            LOG_DEBUG() << state.detected_sync_slaves.size() << " sync slaves detected";
        }
        role_check_guard.Release();
    } catch (const ConnectionError& e) {
        LOG_LIMITED_WARNING() << "Broken connection with " << DsnCutPassword(dsn) << ": " << e;
    }
}
